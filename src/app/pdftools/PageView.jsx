'use client'

import React,{useEffect} from "react";
import imageDataURLFromFile from './methods/imageDataURLfromFile';
import {RandButton} from '../ui/randbutton';
import FileRotateButtons from './components/PDFFile/FilePreviewButtons/FileRotateButtons';

export default function PageView({file, pageNumber, handler, txt}){
  console.log(pageNumber);
    const fileThumbnailRef = React.createRef();
    const PDFThumbnail = React.forwardRef(function Thumb({ src, deg }, ref){
    const getyr = async()=>{
        if (src.image) {
        ref.current.src = src.image;
        return;
        }
    
        const data = await imageDataURLFromFile(src, pageNumber);
        ref.current.src = data;
        src.image = data;

    } 
    useEffect(function(){
      getyr();
    }, []);

    return (
      <div className="grid place-items-center">
        <img
          src="data:image/png;base64,"
          className={` filePreviewThumbnails transition-transform`}
          draggable={false}
          ref={ref}
          style={{
            transform: `rotate(${deg}deg)`,
            width: "120px",
            height: "150px",
          }}
        />
      </div>
    );
  });


  return (
    <div
      className={`flex flex-col h-full w-[200px] border-2 border-green-700 py-0 px-4 m-2 `}
    >
      <span className="z-40 overflow-hidden whitespace-nowrap text-ellipsis relative top-[-10px] left-[-18px] bg-green-700 text-gray-200 text-sm">
        {file.name}
      </span>
        <PDFThumbnail src={file} deg={file.degrees} ref={fileThumbnailRef} />
        <FileRotateButtons file={file} imageRef={fileThumbnailRef} />
        <RandButton text={txt} fun={()=>{handler(file,pageNumber)}} />
    </div>
  );

     
}
